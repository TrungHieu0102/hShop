...
namespace WebApi.Controllers
{
    ...
    public class ProductsController(IProductService productService) : ControllerBase
    {
        ...
        public async Task<ActionResult> AddProduct([FromForm] CreateUpdateProductDto productDto)
        {
            try
            {
                ...
                return !result.IsSuccess
                    ? StatusCode(400, result.Message)
                    : StatusCode(201, new { message = "Product created successfully" });
            }
        ...
        [HttpGet("search")]
        [AllowAnonymous]
        public async Task<IActionResult> SearchProductByName([FromQuery] string name, [FromQuery] int page = 1,
            [FromQuery] int pageSize = 10, [FromQuery] bool IsDecsending = false)
        {
